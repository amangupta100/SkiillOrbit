...
import { userRegisterFields } from "../../InputFields";
...
const page = () => {
  ...
  // Button should be enabled only if all fields are filled, passwords match, password is valid, and not loading
  const canSubmit =
    allFieldsFilled &&
    passwordsMatch &&
    isPasswordValid &&
  ...
  const handleSubmit = async (e) => {
    ...
    // Extra guard (should not be needed, but for safety)
    if (!canSubmit) return;
    try {
      setLoading(true);
      ...
      if (succ) {
        ...
        window.location.href = "/register/job-seeker/profileSetup";
      } else {
        toast.error(message);
      }
    } catch (err) {
  ...
  // Helper to render email verification status
  ...
  return (
    <div className="flex items-center justify-center">
      {showEmailVerifyBox && (
        <EmailVerifyBox email={formData.email} name={formData.fullname} />
      )}
      ...
      <div className="border-[1.6px] border-zinc-200 rounded-lg w-[98%] lg:w-[35%] md:w-[60%] px-5 min-h-fit py-6">
        ...
        <form onSubmit={handleSubmit} className="mt-10">
            ...
            <div key={field.name} className="mb-4">
              ...
              field.name === "confirmPassword" ? (
                <div className="relative">
                  ...
                  {/* Show error if passwords do not match and both fields are filled */}
          ...
          >
            {loading && <ButtonLoader />} Register as Job-Seeker
          </Button>
        </form>
