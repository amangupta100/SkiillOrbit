...
const TooltipTrigger = React.forwardRef(
  (
    { children, delayDuration, hideDelay, asChild = false, triggerRef },
    ref
  ) => {
    ...
    if (align !== "center") {
      if (side === "top" || side === "bottom") {
        style.transformOrigin =
          align === "start" ? "left center" : "right center";
        if (align === "start")
          style.transform =
            "translateX(50%) rotate(45deg)"; // Adjust to push arrow more towards edge
        else style.transform = "translateX(-50%) rotate(45deg)"; // Adjust to push arrow more towards edge
      } else {
        // left or right
        style.transformOrigin =
          align === "start" ? "top center" : "bottom center";
        if (align === "start")
...
export const TooltipContent = React.forwardRef((props, _ref) => {
  console.warn(
    "TooltipContent is deprecated. Use the Tooltip component with content prop instead."
  );
